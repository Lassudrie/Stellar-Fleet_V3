...
## Vue d'ensemble
Ce dossier contient la documentation technique, architecturale et fonctionnelle du projet **Stellar Fleet**, un simulateur de batailles spatiales déterministe.
...
- **[Architecture Système](architecture/system-design.md)** : Architecture globale, séparation Moteur/Données, Stack technique.
...
- **[3D Viewer Foundations](architecture/3d-viewer.md)** : Fondations multi-echelle, rendu multi-pass, transitions sans couture.
- Générateur de surface planétaire : `src/engine/worldgen/planetSurfaceGenerator.ts` (v5 par défaut : macro-continents + rotation/warp + jitter côtier, océans par composante, nettoyage micro-îles; v1/v2 legacy via `scenario.generation.surfaceGeneratorVersion`).
...
- **Source de vérité des types** : décrire les données en s'alignant sur les définitions TypeScript existantes (`src/shared/types.ts`, `src/engine/**`, `src/content/**`). Tout écart doit être explicité, et les sections **Données** et **Constantes** doivent pointer vers les types/constantes correspondants.
