...
- `systems` : liste des systèmes stellaires.
  ...
  - `astro` : bloc astrophysique optionnel (spectral type, étoiles, planètes). Peut être régénéré (voir § 5).
    - `stars[]` : la primaire en premier, suivie des compagnons. Les compagnons peuvent inclure un `orbit` avec `semiMajorAxisAu`, `eccentricity`, `periodDays`, `phaseDeg`, `inclinationDeg`, `ascendingNodeDeg`, `argPeriapsisDeg`, `meanAnomalyAtEpochDeg`.
