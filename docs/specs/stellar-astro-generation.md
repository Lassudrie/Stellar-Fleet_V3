...
- Les compagnons peuvent inclure un `orbit` déterministe (`semiMajorAxisAu`, `eccentricity`, `periodDays`, `phaseDeg`, `inclinationDeg`, `ascendingNodeDeg`, `argPeriapsisDeg`, `meanAnomalyAtEpochDeg`) généré via une RNG dérivée pour ne pas perturber le reste.
- **Planètes (`planets`)** :
