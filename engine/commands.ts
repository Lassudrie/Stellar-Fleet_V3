...
export const applyCommand = (state: GameState, command: GameCommand, rng: RNG): GameState => {
    ...
    switch (command.type) {
            ...
            return {
                ...
                fleets: state.fleets.map(fleet => {
                    ...
                    return {
                        ...
                        loadingTargetSystemId: system.id // Set load order
                    };
                })
            };
        }
        ...
        case 'ORDER_UNLOAD_MOVE': {
            // Functionally identical to INVASION_MOVE, but distinct command for clarity/UI intent
            const system = getSystemById(state.systems, command.targetSystemId);
            ...
            return {
                ...
                fleets: state.fleets.map(fleet => {
                    ...
                    return {
                        ...
                        targetSystemId: system.id,
                        targetPosition: clone(system.position),
                        invasionTargetSystemId: system.id, // Re-use invasion logic for unload
                        loadingTargetSystemId: null
                    };
                })
            };
