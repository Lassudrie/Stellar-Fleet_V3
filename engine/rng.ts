...
export class RNG {
  ...
  public id(prefix: string): string {
    ...
    const fullId = `${prefix}_${uuid}`;

    if (process.env.NODE_ENV !== 'production') {
      if (!this.generatedIds) this.generatedIds = new Set();
      if (this.generatedIds.has(fullId)) {
        logger.warn(`[RNG] Duplicate ID generated: ${fullId}`);
      } else {
        this.generatedIds.add(fullId);
      }
    }
